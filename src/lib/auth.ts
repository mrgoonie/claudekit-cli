...
import { AuthenticationError, type AuthMethod } from '../types.js';
...
export class AuthManager {
  ...
  static async getToken(): Promise<{ token: string; method: AuthMethod }> {
    if (this.token && this.authMethod) {
      return { token: this.token, method: this.authMethod };
    }

    // Try 1: GitHub CLI
    try {
      const token = await this.getFromGhCli();
      if (token) {
        this.token = token;
    ...
    this.token = promptedToken;
    this.authMethod = 'prompt';
