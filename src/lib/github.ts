...
import { GitHubError, GitHubRelease, GitHubReleaseSchema, type KitConfig } from '../types.js';
...
export class GitHubClient {
  ...
  async getLatestRelease(kit: KitConfig): Promise<GitHubRelease> {
    try {
      ...
      if (error?.status === 403) {
        throw new GitHubError('Access denied. Make sure your token has access to private repositories.', 403);
      }
