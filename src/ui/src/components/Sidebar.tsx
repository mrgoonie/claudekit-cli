...
const Sidebar: React.FC<SidebarProps> = ({
	...
	const isProjectConfigView = Boolean(
		currentProjectId && location.pathname === `/config/project/${currentProjectId}`,
	);
	...
	return (
		...
		>
			...
			<div className="px-4 py-2 space-y-1">
				...
				/>
				{/* Project Config when project is selected */}
				{currentProjectId && (
					<SidebarItem
						icon={
							<svg
						...
						isCollapsed={isCollapsed}
						active={isProjectConfigView}
						onClick={() => navigate(`/config/project/${currentProjectId}`)}
					/>
				)}
			</div>
