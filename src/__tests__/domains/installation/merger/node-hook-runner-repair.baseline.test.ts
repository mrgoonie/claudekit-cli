/**
 * Regression baseline: repairClaudeHookCommandPath + repairClaudeNodeCommandPath
 * behavior — all cases locked before Phase 4 edits the originating test file.
 *
 * These cases are copied verbatim from:
 *   src/__tests__/shared/command-normalizer.test.ts
 *
 * If a later phase breaks any case here, it is a regression. Do NOT delete or
 * weaken assertions to fix a failing build — fix the source instead.
 *
 * Note: the phase-01 spec referenced "18 cases from settings-processor.test.ts"
 * but the repair-function cases live in command-normalizer.test.ts (20 cases).
 * All 20 are captured here to maximize regression coverage.
 */
...
// ── normalizeCommand baseline ─────────────────────────────────────────────────

describe("[baseline] normalizeCommand", () => {
...
// ── repairClaudeHookCommandPath — node hook runner baseline ───────────────────

describe("[baseline] repairClaudeHookCommandPath — node hook runner", () => {
...
// ── repairClaudeNodeCommandPath — absolute path branch baseline ───────────────

describe("[baseline] repairClaudeNodeCommandPath — absolute path branch", () => {
	...
	it("matches Windows home dir case-insensitively (lowercased corrupted path)", () => {
		...
		try {
			const h = homedir().replace(/\\/g, "/").replace(/\/+$/, "");
			const lowercased = h.toLowerCase();
