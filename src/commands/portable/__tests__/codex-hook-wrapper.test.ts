...
const legacyCaps: CodexCapabilities =
...
describe("buildWrapperScript", () => {
	...
	it("encodes legacy strip rules for PreToolUse additionalContext", () => {
		const script = buildWrapperScript("/hook.cjs", legacyCaps);
		expect(script).toContain("PreToolUse");
		expect(script).toContain("additionalContext");
	});

	it("does not strip additionalContext for current PreToolUse", () => {
		const script = buildWrapperScript("/hook.cjs", caps);
		...
		expect(scrubRules.PreToolUse.deleteFields).not.toContain("additionalContext");
	});
	...
	it("hook emits valid JSON with deny → scrubbed and passed through, exit 0", () => {
		...
		writeFakeOriginalHook(originalHook, {
			...
			stdout: JSON.stringify({
				...
				additionalContext: "should-be-preserved",
			}),
		});
