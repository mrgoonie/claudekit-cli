...
describe("PathResolver", () => {
	...
	describe("isPathSafe", () => {
		...
		it("should reject Windows UNC paths", () => {
			expect(PathResolver.isPathSafe("\\\\server\\share")).toBe(false);
			expect(PathResolver.isPathSafe("\\\\192.168.1.1\\data")).toBe(false);
			expect(PathResolver.isPathSafe("\\\\domain\\path\\to\\file")).toBe(false);
		});
	});
